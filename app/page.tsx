  ...
  return <main className="app-shell">
    ...
    {active && draft && <div className={`modal-backdrop ${modalClosing ? "closing" : ""}`} onMouseDown={(e) => e.target === e.currentTarget && closeModal()}><section className="modal" role="dialog" aria-modal="true">
      ...
      {editing ? <form className="report-form" onSubmit={(e) => { e.preventDefault(); save(); }}>
        ...
        {(draft.platform === "Instagram" || draft.platform === "TikTok") && <fieldset className="social-comments"><div className="social-comments-head"><div><legend>{t.commentTopics} <span>{t.optional}</span></legend><p>{t.commentTopicsHint}</p></div><label className="grade-field"><span>{t.grade}</span><span><input type="number" min="1" max="10" step="1" value={draft.grade ?? ""} onChange={(event) => setDraft({ ...draft, grade: event.target.value === "" ? null : Number(event.target.value) })} /><b>{t.gradeOutOfTen}</b></span></label></div><div>
          <label><span>{t.brandValue}</span><AutoGrowTextarea value={draft.brandValue} onChange={(brandValue) => setDraft({ ...draft, brandValue })} placeholder={t.brandValuePlaceholder} /></label>
          <label><span>{t.salesValue}</span><AutoGrowTextarea value={draft.salesValue} onChange={(salesValue) => setDraft({ ...draft, salesValue })} placeholder={t.salesValuePlaceholder} /></label>
          <label><span>{t.entertainmentValue}</span><AutoGrowTextarea value={draft.entertainmentValue} onChange={(entertainmentValue) => setDraft({ ...draft, entertainmentValue })} placeholder={t.entertainmentValuePlaceholder} /></label>
        </div></fieldset>}
        ...
        {(active.platform === "Instagram" || active.platform === "TikTok") && active.grade !== null && <div className="detail-grade"><span>{t.grade}</span><strong>{active.grade}<small>/10</small></strong></div>}
